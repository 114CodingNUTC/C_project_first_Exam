...
- 所有測試檔案（`test_*.c` 和 `test_*.exe`）放在 `tests/`；測試代碼遵循同等品質標準。
...
## Build and Test
- Dev C++：以專案 Build & Run 為主流程，輸出 `game.exe`。
- 命令列（MinGW/gcc）：`gcc -o game.exe src/*.c -Iinclude`。
- 執行：`game.exe`。
- **測試檔案管理**：
  - 所有測試程式碼放在 `tests/` 資料夾。
  - 測試編譯命令例：`gcc -o tests/test_phase_b.exe tests/test_phase_b.c src/game.c src/board.c src/rules.c src/messages.c -Iinclude`。
  - 新增 Phase 測試時，使用 `test_phase_X.c` 命名規範。
- 新增規則或輸入邏輯時，請至少做最小 smoke 驗證（勝利、和局、非法落子）。
